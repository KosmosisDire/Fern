...
void FlirPrettyFormatter::visit(FlirStaticAddr* node)
{
    out << "&" << (node->field ? node->field->name : "?");
}
...
std::string FlirDebugFormatter::local_label(FlirLocal* local)
{
    ...
    std::string place = local->byAddress ? std::string("@addr") : std::format("@{}", local->offset);
    return std::format("\"{}\": {} {}", name, local->type ? format_type(local->type) : "?", place);
}
...
void FlirDebugFormatter::visit(FlirStaticAddr* node)
{
    std::string field = node->field
        ? std::format("\"{}\": {}", symbol_label(node->field), format_type(node->field->type))
        : std::string("null");
    begin_node(node, std::format("static: {}, {}", field, type_attr(node)));
}
...
std::string FlirDebugFormatter::format(FlirMethod* method)
{
    ...
    if (method->sretParam)
        fmt.out << ", sret: " << (method->sretParam->type ? format_type(method->sretParam->type) : "?");
    fmt.out << ")";
    fmt.open_block();
    fmt.write_indent();
    fmt.out << "locals: [";
